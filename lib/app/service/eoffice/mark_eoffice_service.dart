...
class MarkEofficeService {
  ...
  Future<MarkEoffice> fetchMarkEoffice(
    int eOfficeId,
    int messageId,
    ...
    try {
      ...
      final response = await http.Client()
          .post(
            Uri.parse(IOT_REQUEST_URL + 'fetchMarkEoffice'),
            headers: {
              "Authorization": "Bearer " + wsToken,
              "Vendor": codec.encode(IOT_APP_VERSION),
            },
  ...
  Future<bool> saveMarkEoffice(
    ...
    try {
      ...
      final response = await http.Client()
          .post(
            Uri.parse(IOT_REQUEST_URL + 'saveMarkEoffice'),
            headers: {
              "Authorization": "Bearer " + wsToken,
              "Vendor": codec.encode(IOT_APP_VERSION),
            },
  ...
  Future<List<MarkEoffice>> searchMarkEoffices(
    ...
    try {
      ...
      final response = await http.Client()
          .post(
            Uri.parse(IOT_REQUEST_URL + 'searchSelfMarkEoffice'),
            headers: {
              "Authorization": "Bearer " + wsToken,
              "Vendor": codec.encode(IOT_APP_VERSION),
            },
  ...
  Future<IotDownloadFile> downloadDataFiles(String markDocId) async {
    try {
      ...
      final response = await http.Client().post(
        ...
        body: jsonEncode({'markDoc': '$markDocId'}),
      );
      if (response.statusCode != 200)
        throw IotException(
          code: response.statusCode,
