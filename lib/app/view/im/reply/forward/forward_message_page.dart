...
class _IotForwardMessagePageState extends State<IotForwardMessagePage> {
  ...
  Widget build(BuildContext context) {
    return Container(
      child: StreamBuilder(
        ...
        builder:
            (BuildContext context, AsyncSnapshot<List<IotPosition>> snapshot) {
              return Column(
  ...
  Widget _buildForwardUsers() {
    var _positions = widget.positionStream.positions
        ...
        onPressed: () async => await Navigator.push(
          ...
          MaterialPageRoute(
            builder: (BuildContext context) =>
                IotListPositionsPage(widget.positionStream),
          ),
        ),
      ),
    );
  ...
  Widget _forwardSendButton() {
    return IconButton(
      ...
      icon: const Icon(Icons.send, size: 32, color: IOT_BG_COLOR),
    );
  }

  Future<void> _sendForwardMessage() async {
    var _receivers = widget.positionStream.positions.where(
