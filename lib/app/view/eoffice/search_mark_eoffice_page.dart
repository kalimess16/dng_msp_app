...
import 'package:dngmsp/app/view/eoffice/result_mark_eoffice_page.dart';
...
class SearchMarkEofficePage extends StatefulWidget {
  @override
  _SearchMarkEofficePageState createState() => _SearchMarkEofficePageState();
}

class _SearchMarkEofficePageState extends State<SearchMarkEofficePage> {
  ...
  Future<void> _search() async {
    ...
    Navigator.push(
        context,
        MaterialPageRoute(
            builder: (BuildContext context) => ResultMarkEofficePage(
                _selectedFromDate,
                _selectedToDate,
                _controller.text,
                _selectedNote ? 'Y' : 'N',
  ...
  Widget _buildChosenDateTime() {
    return StreamBuilder(
        ...
        builder: (BuildContext context, AsyncSnapshot snapshot) {
          if (snapshot.hasData) {
            _selectedDateTime = snapshot.data[0];
          }
          return Container(
  ...
  Widget _buildDateTime() {
    return StreamBuilder(
        ...
        builder: (BuildContext context, AsyncSnapshot snapshot) {
          if (snapshot.hasData) {
            List<DateTime> _dates = snapshot.data[1];
            _fromDate = _dates[0];
            _toDate = _dates[1];
          }
          return Container(
  ...
  Widget _buildChosenNote() {
    return StreamBuilder(
        stream: _stream,
        builder: (BuildContext context, AsyncSnapshot snapshot) {
          if (snapshot.hasData) {
            _selectedNote = snapshot.data[2];
          }
          return Container(
            width: double.infinity,
          ...
          child: Row(
              ...
              children: [
                StreamBuilder(
                    ...
                    builder: (BuildContext context, AsyncSnapshot snapshot) {
                      if (snapshot.hasData) {
                        _selectedAgency = snapshot.data[3];
                      }
                      return Flexible(
                        child: Text(
                          '$_selectedAgency',
