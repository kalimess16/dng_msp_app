    ...
    if ([info respondsToSelector:isiOSAppOnVisionSelector]) {
      NSInvocation *invocation =
          [NSInvocation invocationWithMethodSignature:
                            [info methodSignatureForSelector:isiOSAppOnVisionSelector]];
      invocation.target = info;
      invocation.selector = isiOSAppOnVisionSelector;
      [invocation invoke];
      BOOL isOnVision = NO;
      [invocation getReturnValue:&isOnVision];
      isiOSAppOnVision = [NSNumber numberWithBool:isOnVision];
    }
    NSError *error = nil;
